...
function getReview(reqData, callback) {
   let selectQuery = 'select reserve_user_id, cast(aes_decrypt(user_name, unhex(sha2(?, 512))) as char) as reserve_user_name, ifnull(profile_img_url, ?) as reserve_user_profile_img_url, stars, content, date_format(assess_time, "%Y-%m-%d %H:%i:%S") as assess_time ' +
                      'from reviews as r join users as u on (r.reserve_user_id = u.user_id) ' +
                      'where stroll_user_id = ?  order by assess_time desc limit ?, ?';

   let selectParams = [aes_key, defaultUserImgUrl, reqData.stroll_user_id, reqData.limit.former, reqData.limit.latter];
